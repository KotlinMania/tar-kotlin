# Claude Code Project Instructions — tar-kotlin

## Project Overview

This is **tar-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`tar`](https://crates.io/crates/tar).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/tar/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/tar>
