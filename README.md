...
Kotlin Multiplatform line-by-line clean-room port of the Rust crate [`tar`](https://crates.io/crates/tar).

> **Status: scaffold — porting has not started.** This repo currently contains build infrastructure only. The upstream Rust source for the `tar` crate will be cloned into `tmp/` (gitignored) when porting begins.
...
```kotlin
dependencies {
    implementation("io.github.kotlinmania:tar-kotlin:0.1.0-SNAPSHOT")
}
```
...
Original work copyrighted by the upstream `tar` authors. Kotlin port copyright (c) 2026 Sydney Renee and The Solace Project.
